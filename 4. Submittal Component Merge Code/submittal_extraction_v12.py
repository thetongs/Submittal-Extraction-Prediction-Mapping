...
#
def find_section_name_section_number(section_details):
    global spec_name
    global spec_number
...
def prediction_model(big_spec_name):
    ...
    new_dataset.to_csv(updated_big_spac_name, index = False)
    

## Submittal Extraction 
#
def submital_extraction_mapping(pre_data, process_flag, file_name, big_spec_name):
    # print(pre_data, process_flag, file_name, big_spec_name)
    # # try:
    if(process_flag):
        ...
        for index, line in enumerate(data.splitlines()):
            if("END OF SECTION" in line or "END OF DOCUMENT" in line.strip()):
                ...
                continue
            elif([ele for ele in section_details1 if(ele in line.strip())]):
                continue
            else:
        ...
        for line in final_data.splitlines():
            ...
            if(line.startswith("PART")):
                ...
                final_lines.append(line)
                index = index + 1
            elif(re.search(r"^[A-Za-z]{1,2}\s*\.\s*", line)):
                final_lines.append(line)
                index = index + 1
        ...
        for line in final_lines:
            if(re.search(r"^[0-9]+\.[0-9]+\s", line) or line.startswith("PART")):
                heading.append(line)
                flag = False
            ...
            for index, line in enumerate(dataset): 
                if(index == 0):
                    ...
                    try:
                        ...
                        dictionary[head][cnt]["Submittal Description"] = [line]
                    cnt2 = cnt
                    cnt = cnt + 1
                elif(re.search(r"^[0-9]+\.", line)):
                    try:
                        dictionary[head][cnt2]["Submittal Description"].append(line)   
                    except Exception as e:
                        cnt2 = cnt
                        cnt = cnt + 1
                        dictionary[head].append({"Part" : part_name})
                        ...
                        dictionary[head][cnt2]["Submittal Description"] = [line]
