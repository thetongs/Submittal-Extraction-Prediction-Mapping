...
from submittal_extraction_v16 import submital_extraction_mapping
from submittal_extraction_v16 import convert_to_production_format
from submittal_extraction_v16 import prediction_model

from combine_individual_spec_preprocessing_v16 import combine_spec_data_to_txt
from combine_individual_spec_preprocessing_v16 import create_individual_spec_data_dict
...
logger = logging.getLogger('submittal_ui_v16.py')
    ...
    if(variable.get() == "Combine Spec"):
        ...
        if(s_filepath_spec_pdf and d_filepath_folder):
            ...
            if(len(d_filepath_folder + "/" + big_spec_name + "/" + big_spec_name + ".csv") < 259):
                ...
                # For Each Individual Spec Found
                for ind_section_name, pre_data in individual_specification.items():
                    # Function Call - Submittal Mapping to CSV
                    submital_extraction_mapping(pre_data, ind_section_name, big_spec_name, d_filepath_folder)
        ...
        if(s_filepath_folder and d_filepath_folder):
            ...
            if(len(d_filepath_folder + "/" + big_spec_name + "/" + big_spec_name + ".csv") < 259):
                ...
                for root, dirs, files in os.walk(s_filepath_folder):
                    for file1 in files:
                        if(str(file1).endswith(".pdf")):
                            i_file_path = os.path.join(root, file1)
                ...
                # Rename Log File and Store in Destination Folder Selected
                shutil.copy("logs.log", d_filepath_folder + "/" + big_spec_name + " - DUMP" + "/" + "{}.log".format(big_spec_name))
            else:
                messagebox.showwarning("Warning", "File Path Exceeding 259 Char | Please Select Short Path") 
        else:
            messagebox.showwarning('Warning','Please Select Source PDF File and Destination Folder.')
