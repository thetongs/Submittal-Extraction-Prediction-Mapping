...
import os
logger = logging.getLogger('submittal_ui_v15.py')
...
def prediction_model(big_spec_name, d_filepath_folder):
    ...
    new_dataset = pd.read_csv(d_filepath_folder + "/" +  big_spec_name + " - DUMP" + "/" + "Production_" + big_spec_name + ".csv")
    ...
    new_dataset.to_csv(d_filepath_folder + "/" +  big_spec_name + " - DUMP" + "/" + updated_big_spac_name + ".csv", index = False)
    
# Convert Mapped Data into Production Uniform Format
def convert_to_production_format(d_filepath_folder, big_spec_name):
    dataset_model = pd.read_csv(d_filepath_folder + "/" + big_spec_name + " - DUMP" + "/" + big_spec_name + ".csv")
    ...
    for spec_number, spec_name, part, subsection, subsection_name, description in zip(dataset_model['SECTION'], dataset_model['SECTION NAME'], dataset_model['PART'], dataset_model['SUBSECTION'], dataset_model['SUBSECTION NAME'], dataset_model['DESCRIPTION']):
        ...
        splitted_dataset.to_csv(d_filepath_folder + "/" + big_spec_name + " - DUMP" + "/" + "Production_" + big_spec_name + ".csv", index = False)
    ...
    if(point_flag == True):
        ...
        if(submittal_present_flag):
            ...
            if(flag_of_section):    
                ...
                store_path = d_filepath_folder + "/" + big_spec_name + " - DUMP" + "/" + big_spec_name + ".csv"
                ...
                if(os.path.exists(store_path)):
                    ...
                    os.mkdir(d_filepath_folder + "/" + big_spec_name + " - DUMP" + "/")
